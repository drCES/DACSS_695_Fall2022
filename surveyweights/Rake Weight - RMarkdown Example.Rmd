...
```{r , echo=FALSE}
sample <- read_dta("C:/Users/Carey/Dropbox/UMass/Surveys Class/Data/sample.dta")
...
View the lecture slides here for discussion on why this could be done. 
...
##We can also bind the two values together to perform significance tests 
##on the differences between the weighted and unweighted sample data
#Shows the weighted size of the age_group demographics
ag_w<-sample %>%
  as_survey(weights = c(weight)) %>%
  group_by(age_group) %>%
...
sample.weighted <- sample %>% 
  as_survey_design(ids = 1, # 1 for no cluster ids; use this for a simple random sample 
                   weights = weight, # No weight added
...
educ <-c(.2975, .2445, .0828, .2398, .1354) #Target values education - HS or less, Some college, AA, BA, Graduate degree
